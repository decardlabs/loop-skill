...
本章展示用户提交三份文件后，spec-forge 完整的工作流程。
...
用户项目/
├── PRD.md              ← 产品需求（要做什么）
├── DESIGN.md           ← 设计 Token（长什么样）
└── 技术栈规范说明.md    ← 技术规范（用什么做、怎么做）
```
...
用户提交三份文件 → /forge
     │
     ├── PRD.md ──────────┐
     ├── DESIGN.md ───────┤──→ 同时被 AI 消费 → 生成对齐所有约束的代码
     └── 技术栈规范说明.md ─┘
                                    │
                                    ▼
     ┌──────────────────────────────────────────────────────────┐
     │  Step 1: DETECT — 加载三份文件                           │
     │                                                          │
     │  spec-forge 启动时自动检测:                                │
     │    ┌─ PRD.md → 读取内容作为 SPEC 阶段输入                  │
     │    │                                                      │
     │    ├─ DESIGN.md → 提取设计 Token:                         │
     │    │   主色: #2563EB                                      │
     │    │   字体: Inter                                        │
     │    │   圆角: 8px                                          │
     │    │                                                      │
     │    └─ 技术栈规范说明.md → 提取技术约束:                      │
     │        前端: TypeScript + React + Vite + Tailwind          │
     │        后端: Go + Gin + GORM                               │
     │        API 格式: {code, message, data}                     │
     └──────────────────────────────────────────────────────────┘
                                    │
                                    ▼
     ┌──────────────────────────────────────────────────────────┐
     │  Step 2: SPEC — 生成技术规格                              │
     │                                                          │
     │  AI 同时消费三份文件，生成一份统一的、对齐所有约束的 SPEC:   │
     │                                                          │
     │  # 技术规格: 用户注册                                     │
     │  ## 架构概览                                              │
     ...
     │  Step 3: PLAN — 拆解任务                                  │
     │                                                          │
     │  每个 task 的 prompt 自动注入该 task 需要的约束:            │
     │                                                          │
     │  TASK-001: 创建 User 数据库表 + GORM 模型                  │
     │  └─ prompt 中包含:                                        │
     │      技术栈: Go + GORM                                    │
     │      表结构: 用户名字段 + 邮箱字段（来自 PRD）              │
     │      设计约束: 无（后端不需要）                             │
     │                                                          │
     │  TASK-002: 实现注册 API（Go + Gin）                        │
     │  └─ prompt 中包含:                                        │
     │      路由: POST /api/v1/auth/register                     │
     │      响应: {code, message, data}（来自技术栈规范）          │
     │      密码: bcrypt 加密                                    │
     │                                                          │
     │  TASK-003: 实现注册页面（React + TypeScript + Tailwind）    │
     │  └─ prompt 中包含:                                        │
     │      主色: #2563EB（来自 DESIGN.md）                        │
     │      圆角: 8px（来自 DESIGN.md）                           │
     │      字体: Inter（来自 DESIGN.md）                         │
     │      组件: 函数组件 + Hooks                               │
     │      API: TanStack Query（来自技术栈规范）                  │
     └──────────────────────────────────────────────────────────┘
                                    │
                                    ▼
     ┌──────────────────────────────────────────────────────────┐
     │  Step 4: FORGE — 执行 + 质量门禁                          │
     │                                                          │
     │  对每个 task:                                             │
     │                                                          │
     │  TASK-001: 创建 User 表                                   │
     ...
     │    └─ go test ./...                                       │
     │    ↓                                                     │
     │    ✅ git commit -m "TASK-002: 注册 API"                   │
     │                                                          │
     │  TASK-003: 注册页面                                       │
     │    AI 写 TypeScript                                       │
     │    ├─ npx tsc --noEmit          ← TS 类型错误拦截          │
     │    ├─ npx eslint src/           ← 风格检查                │
     │    └─ npx jest                  ← 组件测试                │
     │    ↓                                                     │
     │    ✅ git commit -m "TASK-003: 注册页面"                   │
     └──────────────────────────────────────────────────────────┘
                                    │
                                    ▼
     ┌──────────────────────────────────────────────────────────┐
     │  Step 5: REPORT — 用户看到的执行报告                       │
     │                                                          │
     │  # 执行报告: my-project                                   │
     │                                                          │
     │  ## 概览                                                  │
     ...
     │  |------|-------|--------|-------|                        │
     │  | TASK-001 | User 表 | a1b2c3d | ✅ |                    │
     │  | TASK-002 | 注册 API | e4f5g6h | ✅ |                   │
     │  | TASK-003 | 注册页面 | i7j8k9l | ✅ |                   │
     │                                                          │
     │  ## 类型检查                                              │
     │  | Task | TypeScript | Go | Lint |                        │
     │  |------|-----------|----|------|                        │
     │  | TASK-001 | — | ✅ go vet | ✅ |                       │
     │  | TASK-002 | — | ✅ go vet | ✅ |                       │
     │  | TASK-003 | ✅ tsc | — | ✅ |                          │
     │                                                          │
     │  ## Git 分支                                              │
     │    全部成功 → git merge auto/task-run-<ts>                │
     └──────────────────────────────────────────────────────────┘
                                    │
                                    ▼
                         用户拿到的代码:
                         ...
                         ├─ 符合技术栈规范的技术选型
                         ├─ 通过类型检查 + lint + 测试
                         └─ 每个 task 独立 git commit
```

#### 10.3 三份文件在流程中的角色

```
                  PRD.md              DESIGN.md         技术栈规范说明.md
                    │                    │                    │
                    ▼                    ▼                    ▼
      ┌──────────────────────────────────────────────────────────┐
      │                  SPEC 阶段                                │
      │  AI 同时消费三份文件，生成对齐所有约束的技术规格              │
      │  "用 Go+GORM 实现 PRD 里的用户注册，UI 用 #2563EB 主色"    │
      └─────────────────────┬────────────────────────────────────┘
                            │
                            ▼
      ┌──────────────────────────────────────────────────────────┐
      │                  PLAN 阶段                                │
      │  每个 task 的 prompt 注入该 task 需要的约束                 │
      │  前端 task → DESIGN.md token                              │
      │  后端 task → Go/GORM 规范                                 │
      │  数据库 task → PostgreSQL + 表结构                         │
      └─────────────────────┬────────────────────────────────────┘
                            │
                            ▼
      ┌──────────────────────────────────────────────────────────┐
      ...
      │  AI 在约束下生成代码                                       │
      │  类型检查 → lint → 测试 → commit                           │
      │  全部对齐 PRD.md + DESIGN.md + 技术栈规范说明.md            │
      └──────────────────────────────────────────────────────────┘
                            │
                            ▼
                     用户拿到的代码:
...
错误拦截层级（越左越好）:
  ...
  编译时  ←──  风格  ←──  测试  ←──  审查  ←──  线上
    ▲        ▲        ▲        ▲        ▲
    │        │        │        │        │
  tsc --noEmit  ESLint  pytest  人工    灾难
  go vet      gofmt   go test  review

  ───────────────────────────────────────────
  spec-forge 覆盖    覆盖    覆盖    未覆盖    ❌
  ✅                  ✅     ✅
```

三层门禁（类型检查 → lint → 测试）确保 AI 生成的代码在进入代码库之前被全面检查：

| 门禁 | 拦截什么 | AI 常见错误举例 |
|------|---------|---------------|
| **类型检查** | 类型错、接口错、未使用变量 | 函数签名不匹配、null 引用、拼错的字段名 |
| **Lint** | 风格不一致、未使用的导入 | 缩进混乱、多余的 import、命名不规范 |
| **测试** | 业务逻辑错误 | API 返回格式不对、数据库操作遗漏 |

**类型检查是最重要的一道门禁。** TypeScript strict 模式和 Go 的编译时检查意味着 AI 生成的代码在编译阶段就能暴露大部分错误——不需要等到运行时才发现。这是"帮用户用好 AI 编程"这一目标在技术层面的核心实现。
